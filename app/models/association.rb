...
class Association < ApplicationRecord
  ...
  with_options if: -> { form_step == 'foreign_key' } do
    ...
    validates :nature, presence: true
    validates :foreign_key_location_model, presence: true
    validates :foreign_key, presence: true, exclusion: { in: %w[id], message: 'with that name not allowed' }
  end
  ...
  def one?
    if direct?
      ...
      source_association.one? && through_association.one?
    end
  end
  ...
  def indirect_many_one?
    indirect? && through_association.many? && source_association.one?
  end

  def indirect_many_many?
    indirect? && through_association.many? && source_association.many?
  end

  def indirect_one_one?
    indirect? && through_association.one? && source_association.one?
  end

  def indirect_one_many?
    indirect? && through_association.one? && source_association.many?
  end

  def parent_model
    if foreign_key_location_model == origin_model
      terminus_model
    elsif foreign_key_location_model == terminus_model
      origin_model
    end
  end

  def can_eliminate_class_name?
    name.classify == if belongs_to?
                       parent_model.class_name
                     else
                       foreign_key_location_model.class_name
                     end
  end

  def can_eliminate_foreign_key?
    if belongs_to?
      foreign_key.chomp('_id') == name
    else
      foreign_key.chomp('_id').classify == origin_model.class_name
    end
  end
end
